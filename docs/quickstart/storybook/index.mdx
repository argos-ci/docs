...
If you use Test Runner instead of Vitest, follow our [Storybook Test Runner quickstart](/quickstart/storybook-test-runner).<br/>
If you use a legacy version of Storybook (\<v8), follow our [legacy Storybook quickstart](/quickstart/storybook-legacy).
...
`ARGOS_TOKEN` is the project token from **Settings → General → Token**. On GitHub Actions, you can also use [OIDC](/github-oidc) or [tokenless authentication](/github-tokenless) to avoid managing a secret.

For other CI providers, follow [Use deployments in CI](/deployments/ci).
