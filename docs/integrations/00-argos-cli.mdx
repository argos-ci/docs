...
The Argos Command Line Interface (CLI) is a tool that allows you to easily send screenshots to the Argos app. It can be used locally, but it is primarily intended to be run by a continuous integration (CI) system.
...
To install `@argos-ci/cli`, run the following command:
...
In order to use `@argos-ci/cli`, you need to configure your repository token as an environment variable.
...
Once configured, the `argos upload` command can be run multiple times to aggregate screenshots in the same build
