...
Use the `deploy` command to deploy a static build (Storybook or any static site) to Argos. See [Deployments](/deployments) for an overview of the feature.
...
If the branch matches the project's production branch pattern, the deployment is created as production even without `--prod`. See [Environments](/deployments/environments) for the full rules.
...
3. [GitHub OIDC](/github-oidc) or [tokenless authentication](/github-tokenless) on GitHub Actions.
