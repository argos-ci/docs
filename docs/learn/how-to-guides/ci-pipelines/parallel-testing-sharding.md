...
Set `ARGOS_PARALLEL_TOTAL` to `-1`. Each upload joins the build as a shard, and the build stays open until you explicitly close it by running [`argos finalize`](../../../sdks-reference/argos-command-line-interface-cli.md#uploading-from-ci) in a step that runs after all uploads.
