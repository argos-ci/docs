---
description: Subscribe to a paid Argos plan via Stripe when your invoiced GitHub account cannot use the GitHub Marketplace.
---

# Invoiced customers

Discover how invoiced GitHub accounts affect subscribing to Argos's paid plan and explore alternative subscription options.

If you have a GitHub invoiced account and are looking to subscribe to Argos's paid plan through the GitHub Marketplace, you may have encountered the following error message:

> "Unfortunately, invoiced customers cannot purchase paid plans on the GitHub Marketplace."
>
> **— GitHub error message**

This error message can be confusing, but it simply means that if your GitHub account is set up to receive an invoice for your subscription rather than charging your credit card directly, you won't be able to use the GitHub Marketplace to subscribe to Argos.

Not to worry! You can still purchase a paid Argos plan by [subscribing via Stripe](how-to-subscribe.md).
